...
The task file holds only what GitLab cannot know: `next`, `blocked_on`, `title`, `docs`, notes, the branch, the
...
GitLab answers live in `local-docs/.flow-cache/forge.json`. `flow sync` refreshes them, and so do online
...
Only resolvable threads count. Bot threads count too, because GitLab blocks the merge on them. When the computed
...
rebase, reset, merge) and `origin/<branch>` (push); MR events from GitLab (created, merged, ready/draft, review
...
| **Automation** | SessionStart prints `flow status --brief` into the context; `flow status`/`flow next` read MR state from GitLab | the hook `flow install-hook` adds |
...
| changes → review-wait | agent, `flow next` | gate + push, as above; the stage changes once GitLab sees the reply |
...
When GitLab is unreachable (`glab` timeout), transitions that depend on it refuse with a reason; `flow status` shows
`!N?` (a yellow MR badge on the board).
...
5. **Merge.** The human merges in GitLab; after the next sync the task becomes `merged`.
...
8. **Board.** `flow board` (= `flow status --html --open`) rebuilds and opens `local-docs/flow-status.html`: a
   ...
   without an update), MR state (a GitLab link), and `blocked_on` chips linking to the cards of known keys. Click
...
`--no-gate --why "…"`. The forge is GitLab only, through `glab`.
...
- `flow doctor [--fix]` — symlinks in every worktree (`--fix` relinks broken ones), branches without a task file,
  ...
  closed MR, a stale pin, legacy fields, a missing `local-docs/` in the primary checkout, glab availability.

## Safeguards

- `flow clean` does not delete commits that are on no remote and not in the head of the merged MR, even with
  `--force-unmerged`; to do that deliberately, pass `--discard-commits`. A branch GitLab deleted after the merge
...
- `flow next` in review runs the gate and checks in the worktree where the branch is actually checked out, not the
  ...
  on the server.
- A glab 403 gets its own access message rather than "forge unavailable", and the process's other calls carry on.
...
4. **The journal is written by `sync`, `log`, and `clean`, not by the hook.** Without GitLab access and without
